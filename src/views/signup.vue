  ...
  <div class="h-screen bg-mine flex items-center justify-center pt-10 lg:pl-18">
    <div class="bg-mine shadow-lg rounded-xl p-6 sm:p-8 max-w-sm sm:max-w-md w-full">
      ...
      <div class="flex justify-between mb-6 space-x-4 gap-5">
        <button @click="handleGoogleSignIn" v-if="!isStillLoading" class="hover:scale-95 transition-transform duration-200 sm:pl-5">
          <img :src="logo1" class="pl-[70px] h-13 mt-[-10] sm:mt-0.5 mr-5 w-43.25">
        </button>
      ...
      <form @submit="handleEmailSignIn">
        <div class="relative w-full mb-4">
          <img :src="logo3" class="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5">
          <input v-model="email" type="email" placeholder="Your Email" class="w-full p-3 pl-10 bg-rine rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-400">
        </div>

        <div class="relative w-full mb-4">
          <img :src="logo4" class="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5">
          <input v-model="password" type="password" placeholder="Your Password" class="w-full p-3 pl-10 rounded-lg focus:outline-none bg-rine focus:ring-2 focus:ring-purple-400">
        </div>
        ...
        <div class="flex justify-center">
          <button v-if="!isLoading" type="submit" class="focus:scale-95 transition-transform duration-200">
      ...
      <p class="pl-[16px] mb-[10px] mt-2">
        Don't have an account? <RouterLink to="/createaccount" class="text-tine font-bold hover:underline">Create One Here</RouterLink>
      </p>
    </div>
  </div>
</template>